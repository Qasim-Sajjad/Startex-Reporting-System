...

{
    ...
    public function store(Request $request)
    {
        ...
        $format = Format::create([
            'user_id' => $userId,      // Assign user_id to the format
...

    {
        ...
        return view('superadmin.createformat', compact('users'));
    }
...

    {
        ...
        $formats = Format::where('user_id', $request->user_id)->get();
...

    {
        ...
        assignshops::where('format_id', $request->format_id)->delete();
        ...
        try {
            ...
            return response()->json(['error' => 'Failed to delete format: ' . $e->getMessage()], 500);
        }
    }
...

    {
        ...
        $format = Format::with('sections.questions.options', 'sections.questions.keywords')->findOrFail($id);

        //    dd( $format);

        return view('superadmin.editformat', compact('format'));
    }
    ...
    //         'section_id' => $section->id,             // Return the question ID
    //     ]);
    // }
    public function saveSection(Request $request)
    {
        ...
        if (!$section) {
            ...
            $maxOrderBy = Section::where('format_id', $request->format_id)->max('orderby');

            // Set the new section's orderby to maxOrderBy + 1 if sections exist, otherwise set it to 1
            $section->orderby = $maxOrderBy ? $maxOrderBy + 1 : 1;
        } else {
    ...
    {
        // Check if the question exists for the given section
        ...
        if (!$question) {
            $question = new Question();
            $maxOrder = Question::where('section_id', $request->section_id)
                ->max('orderby');  // Get the max order value in the section

            $question->orderby = $maxOrder ? $maxOrder + 1 : 1;
        }

        // Assign attributes
        $question->question_name = $request->question_name;
        $question->section_id = $request->section_id; // Use section_id from request
        $question->guideline = $request->guideline;   // Assign guideline (can be null or missing)
        $question->score = $request->score;           // Assign score (can be null or missing)
        $question->type = $request->question_type;    // Assign question type from request
        $question->save();

        // Return response with question ID and a success message
        return response()->json([
            'success' => 'Question saved successfully!',
            'question_id' => $question->id,             // Return the question ID
        ]);
    }
    ...
    public function saveOption(Request $request)
    {
        ...
        // Try to find the option by ID if provided
        ...
        if ($option) {
            ...
            // If option_id is not provided or option doesn't exist, create a new one
            $option = new Option();
            $option->question_id = $request->question_id; // Use the question_id from the request
            $option->text = $request->text;
            $option->score = $request->score;
        }

        // Save the option (either new or updated)
        $option->save();
